...
### Прежние адреса дисциплин

Дисциплины объединяются и переименовываются, а ссылки на них живут в чатах
и презентациях. Прежние адреса перечисляются в `catalog/site.json` полем
`aliases`; [DisciplinePage.jsx](../src/pages/DisciplinePage.jsx) по ним
перенаправляет на новый раздел, а незнакомый адрес показывает страницу
«Раздел не найден» — раньше он молча открывал первую дисциплину списка,
и человек думал, что попал куда надо.
...
Ссылка на электронную библиотеку МГТУ стоит в подвале каждой страницы
([Layout.jsx](../src/components/Layout.jsx)) — на страницах дисциплин её
показывали ещё и полосой сверху, но рядом со списком литературы это оказалось
повтором.

Поле `description` — пояснение строкой под названием работы; годится и обычным
материалам, не только ссылкам.

Поле `note` — заметка о работе («32 варианта», «17 лекций, два модуля»).
Раньше эти строки писались в `deadline`, и карточка рисовала рядом часы, будто
это срок сдачи; `deadline` остался пояснением к дате из `due`.

Поле `layout: "groups"` меняет вид списка файлов: вместо строк с размером
и кнопками карточка рисует плитки с подписями из `label` — так показываются
варианты, у которых свой файл на каждую учебную группу. Двадцать групп списком
занимают пол-страницы, плитками — четыре строки, и студент ищет глазами
знакомое название, а не читает одинаковые строки.
...
файла нет — справа остаётся одна ссылка на каталог.
...
Кнопка не перезагружает текущую страницу, а уводит на главную и грузит её
заново: раздела, который человек читал, в новой версии может уже не быть —
работа переехала, дисциплины объединились. К адресу добавляется метка
`?updated=<время>`, иначе браузер отдаст главную из того же кэша, из-за
которого страница и устарела; сразу после загрузки метка убирается из адреса
([main.jsx](../src/main.jsx)), чтобы не мозолила глаза.
