...
│   ├── lama-logo.svg              # полный логотип лаборатории (вектор)
│   ├── lama-logo.png              # он же, отрисованный в большом размере
...
математики» — нарисован в векторе: [assets/lama-logo.svg](../assets/lama-logo.svg).
Скрипт растеризовать SVG не умеет, поэтому рядом лежит его отрисовка
в большом размере — `assets/lama-logo.png` (прозрачный фон, 996×1787);
из неё и делается `public/logo-full.png`. Если логотип поменяется, отрисуйте
вектор в PNG любым способом (в macOS хватает `qlmanage -t -s 2000`), положите
вместо `assets/lama-logo.png` и выполните `npm run icons`.
